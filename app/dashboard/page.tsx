...
export default function Dashboard() {
  ...
  // 1) Dosya secilince: yuklemeden once onizleme goster
  ...
  // 3) Iptal: onizlemeyi at, yukleme yapma
  ...
  async function removeAvatar() {
    ...
    try {
      ...
      // storage'daki dosyayi da temizlemeye calis (varsa)
      try {
        const { data: list } = await supabase.storage.from("avatars").list(user.id);
        if (list && list.length) {
          await supabase.storage.from("avatars").remove(list.map((f: any) => `${user.id}/${f.name}`));
        }
      } catch {}
      setAvatarUrl("");
      setProfileMsg(t.photoRemoved);
        ...
        <div className={cardCls}>
          ...
          <div className="flex items-center gap-4 mb-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              {pendingFile ? (
                <>
                  <div className="text-[11px] text-sky-400 font-bold">👁️ {t.preview}</div>
                  <div className="flex gap-2">
                    <button onClick={confirmAvatar} disabled={uploadingAvatar} className="text-sm font-bold px-4 py-2 rounded-xl bg-gradient-to-r from-sky-600 to-sky-500 hover:opacity-90 transition disabled:opacity-50">
                      {uploadingAvatar ? t.uploading : `✓ ${t.confirmPhoto}`}
                    </button>
                    <button onClick={cancelAvatar} disabled={uploadingAvatar} className="text-sm font-bold px-4 py-2 rounded-xl bg-white/10 hover:bg-white/15 border border-white/15 transition disabled:opacity-50">
                      {t.cancelPhoto}
                    </button>
                  </div>
                </>
                ...
                <div className="flex gap-2">
                  <button onClick={() => fileRef.current?.click()} className="text-sm font-bold px-4 py-2 rounded-xl bg-white/10 hover:bg-white/15 border border-white/15 transition">
                    📷 {t.photo}
                  </button>
                  {avatarUrl && (
                    <button onClick={removeAvatar} disabled={uploadingAvatar} className="text-sm font-bold px-4 py-2 rounded-xl bg-white/5 hover:bg-red-500/15 border border-white/10 text-zinc-400 hover:text-red-400 transition disabled:opacity-50">
                      🗑️ {t.removePhoto}
                    </button>
                  )}
                </div>
